...
const rvmAllRubyVersions = 'bash -l -c "rvm list known"';
const rvmLocalRubyVersions = 'bash -l -c "rvm list"';
const rvmSetDefault = 'bash -l -c "rvm --default use ';
const versionPattern = /([0-9]+(?:\.[0-9]+)+)/g;
const nodeVersionPattern = /([0-9]+(?:\.[0-9-a-z]+)+)/g;
const npmGlobalsList = 'npm ls -g';
const readline = require('readline');
const mavenDownloadLinuxMac = /http[^"]+maven-([0-9.]+)-bin\.tar\.gz/g;
const mavenDownloadWindows = /http[^"]+maven-([0-9.]+)-bin\.zip/g;
const mavenUrl = 'https://maven.apache.org/download.cgi';
const checkMavenVersion= 'mvn -v';
const npmInstallGlobal = 'npm install -g';
...
let findAndInstallNpmModules = () => {
    ...
    let optionalPrompt = 'do you want to install these npm updates now (y/n)?  ';
    console.log('getting installed node modules.');
    return findUserGlobals(getSystemCmd(npmGlobalsList), getGlobals)
                ...
                return findLatestVersion(packageGlobals.windows, findVersion)
                    .then(windowsPackages => {
                        ...
                        if (userState.windows.required.length > 0) {
                            ...
                            return Promise.race([executeSystemCommand(getSystemCmd(getInstallationCommand(userState.windows.required, npmInstallGlobal, '@')), { resolve: options.resolve }),
                                handleUnresponsiveSystem(30000, 'The system is not responding.\ndo you want to keep waiting (y/n)?  ')]);
                        }
                    })
            }
        })
        .then(() => findLatestVersion(packageGlobals.npm, findVersion)
            .then(npmPackages => {
                ...
                if (userState.npm.required.length > 0) {
                    ...
                    return executeSystemCommand(getSystemCmd(getInstallationCommand(userState.npm.required, npmInstallGlobal, '@')), options);
                }
            }))
        .then(() => {
            if (userState.windows && userState.windows.optional.length > 0) {
                ...
                return confirmOptionalInstallation(() => executeSystemCommand(getSystemCmd(getInstallationCommand(userState.windows.optional, npmInstallGlobal, '@')), options), 'do you want to install these optional windows updates now (y/n)?  ');
            }
        ...
        .then(() => {
            if (userState.npm.optional.length > 0) {
                ...
                return confirmOptionalInstallation(() => executeSystemCommand(getSystemCmd(getInstallationCommand(userState.npm.optional, npmInstallGlobal, '@')), options), 'do you want to install these optional npm updates now (y/n)?  ');
            }
        })
        .then(() => {
...
let checkRubyInstallWindows = () => executeSystemCommand('ruby -v', {resolve: options.resolve})
    .catch(() => {
...
let checkRvmInstallMacLinux = () => executeSystemCommand('which rvm', {resolve: options.resolve})
    .catch(e => {
    ...
    .then(() => {
        ...
        return executeSystemCommand(rvmLocalRubyVersions, getLocalRubyOptions);
    })
    .then(rubyVersion => executeSystemCommand(rvmAllRubyVersions, {resolve: options.resolve})
        .then(allVersions => {
    ...
    .then(rubyVersion => executeSystemCommand(rvmSetDefault + rubyVersion + '"', options)
        .catch(e => {
            console.log(e);
        }).then(() => {
            console.log('ruby install complete. default version is now ' + rubyVersion + '.');
        }));
let displayUserPrompt = displayPrompt => new Promise((resolve, reject) => {
...
let installMaven = () => {
    let downloadPattern = (operatingSystem === 'win32') ? mavenDownloadWindows : mavenDownloadLinuxMac;
...
let convertToBashCommand = systemCommand => 'bash -l -c \"' + systemCommand + '\"';
let getSystemCmd = systemCommand => (operatingSystem === 'win32') ? convertToPowershellCommand(systemCommand) : convertToBashCommand(systemCommand);
            ...
            return executeSystemCommand(portListenCommand, { resolve: options.resolve })
                .then(osResponse => {
                    if (osResponse.includes('ESTABLISHED')) {
                        ...
                        setTimeout(waitForEstablishedConnection, 1000);
                    }
                })
                .catch(() => {
                    console.log('did not find any process at port 4502, checking again.');
                    setTimeout(waitForEstablishedConnection, 5000);
                });
        })();
    });
};
...
let aemInstallationProcedure = function() {
    let folderSeparator = (operatingSystem === 'win32') ? '\\' : '/';
    ...
    return executeSystemCommand('mkdir ' + goUpDirectories(2) + 'AEM', options)
        ...
        .then(() => executeSystemCommand('mkdir ' + goUpDirectories(1) + 't-mobile' + folderSeparator + 'node', options)
            .then(() => {
                ...
                return executeSystemCommand('cp "' + nodePath + '" ' + goUpDirectories(1) + 't-mobile' + folderSeparator + 'node' + folderSeparator, options);
            }))
        .then(() => {
        ...
        .then(() => {
            ...
            return packageArray.reduce((promise, zipFile) => promise.then(() => new Promise((resolve, reject) => {
                (function waitForEstablishedConnection() {
                    let formData = {
                    ...
                    request.post({url: crxInstallUrl, formData: formData}, (err, httpResponse, body) => {
                        if (err) {
                            ...
                            setTimeout(waitForEstablishedConnection, 3000);
        ...
        .then(() => {
            //kill the process aem server for windows
...
let validateRubyInstall = () => (operatingSystem === 'win32') ? checkRubyInstallWindows() : checkRvmInstallMacLinux();
let fullInstall = () => {
    let systemState = {};
    validateRubyInstall()
        .then(() => checkForGemUpdates())
        .then(() => findAndInstallNpmModules())
        .then(userState => {
            ...
            installAngularUiYarn += (operatingSystem === 'win32') ? '; yarn install' : ' && yarn install';
        ...
        .then(angularUiSuccess => {
            if (angularUiSuccess) {
                ...
                console.log('update to webdriver not done. ');
            }
        })
        .catch(error => {
        ...
        .then(() => {
            ...
            return executeSystemCommand('javac -version', javaOptions);
        })
        ...
        .then(() => executeSystemCommand(checkMavenVersion, {resolve: options.resolve}))
        .catch(() => {
