...
let installMavenOnHost = () => {
    ...
    return getVersionsWithRequest(mavenUrl, downloadPattern, packageGlobals.maven)
        ...
        .then(() => {
            ...
            if (operatingSystem === 'win32') {
                let setM2Home = setSystemEnvironmentVariable('M2_HOME', '\'' + unzippedFolderPath + '\'');
                let setMavenHome = setSystemEnvironmentVariable('MAVEN_HOME', '\'' + unzippedFolderPath + '\'');
                let setSystemPath = '$old_path = ' + getSystemEnvironmentVariableForWindows('path') +
                    '; $new_path = $old_path + \';\' + \'' + unzippedFolderPath + '\' + \'\\bin\'; ' +
                    setSystemEnvironmentVariable('path', '$new_path');
                return executeSystemCommand(getSystemCmd(setM2Home + ';' + setMavenHome + ';' + setSystemPath), options);
            } else {
                let outFile = (operatingSystem === 'darwin') ? '.bash_profile' : '.bashrc';
...
let getSystemEnvironmentVariableForWindows = function (variableName) {
    return '[Environment]::GetEnvironmentVariable(\'' + variableName + '\', \'Machine\')';
};
let setSystemEnvironmentVariable = function (variableName, variableValue) {
    return '[Environment]::SetEnvironmentVariable(\'' + variableName + '\', ' + variableValue + ', \'Machine\')';
};
...
let startAemServer = function (commandSeparator, jarName) {
...
let downloadAllAemFiles = function (folderSeparator) {
...
let aemInstallationProcedure = () => {
    ...
    let downloadFile = (dependency, globalPackage) => downloadPackage(globalPackage[dependency], aemFolderPath + folderSeparator + dependency)
        .then(() => dependency);
    let jarName = '';
