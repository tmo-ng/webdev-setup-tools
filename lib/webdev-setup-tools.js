...
function getVariablesFromUser(arrayOfConfigVariables, validateInputFunc) {
...
function getVariablesFromFile(filePath) {
...
function getMissingVariables(setuprcPath, arrayOfConfigVariables) {
...
function shouldModifyGitIgnore(isAGitRepo, gitIgnorePath, fileName) {
...
// validateInputFunc - function used to validate user input
function getConfigVariables(requestedConfigVariables, validateInputFunc) {
    ...
    if (existingSetupRc) {
        ...
        let noMissingValues = variablesToConfigure.length === 0;
        if (noMissingValues) {
            return userVariables;
        }
    }
    return getVariablesFromUser(variablesToConfigure, validationFunction)
        .then(function (userResponseMap) { // write text to file then return user responses
            let fileText = '';
